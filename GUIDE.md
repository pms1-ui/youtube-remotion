...
### hmad 스크립트 추가 지침 (필수 준수)

**서론 후킹 규칙:**
- 인사 직후 시청자가 공감할 수 있는 직격 질문으로 시작 ("왜 ~인데 ~가 안 될까요?")
- 대부분이 겪는 실패/고민 상황을 구체적으로 묘사하여 "나 얘기다" 느낌 유발
- 서론에서 바로 핵심 수치 하나를 제시하여 권위감 확보 ("2022년 연구에서 ~가 37% 차이났습니다")
- "오늘 이 영상 하나로 ~가 달라질 테니 끝까지 집중하시죠" 류의 시청 유도 마무리

**수치/연구 삽입 밀도:**
- 전체 스크립트에 최소 5개 이상의 구체적 수치/연구 결과 포함
- 서론에 1개, 본론 각 파트마다 최소 1개씩, 결론 전 1개
- "~% 증가/감소", "~배 차이", "~그룹 대비" 등 비교 수치 형태 권장
- 수치는 차트/그래프 장면에 바로 활용 가능한 형태로 작성

**자연스러운 읽기 규칙:**
- "1번.", "첫째," 같은 기계적 넘버링 금지 → "자, 첫 번째 운동 갑니다." 식의 구어체 전환
- 운동명/파트명은 문장 안에 자연스럽게 녹이기 ("광배근 하부를 때리는 수정된 랫 풀다운입니다")
- 중간중간 시청자에게 직접 말 거는 큐 삽입 ("다들 있으시죠?", "이거 핵심입니다")
